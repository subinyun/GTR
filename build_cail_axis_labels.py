#!/usr/bin/env python3
"""Build weak Chinese legal-element axis labels for CAIL2018 GTR v2."""
...
from cail_gtr_axis_schema import get_axis_ids, get_axis_schema, get_statute_to_axes_map, save_axis_schema


DEFAULT_DATA_DIR = Path("final_all_data/cail2018_statute_classification")
...
def build_split(
    ...
    *,
    ...
    axis_sources: Dict[str, Counter[str]] = {axis_id: Counter() for axis_id in get_axis_ids()}
    ...
    for idx, row in enumerate(rows):
        ...
        for axis_id, value in labels.items():
            ...
            axis_sources[axis_id][sources[axis_id]] += 1
        for statute in statutes:
            for axis_id, value in labels.items():
                if value == 1:
    ...
    return {
        ...
        "num_unique_statutes": len(label_counts),
        "top_statutes": label_counts.most_common(30),
        "axis_counts": {
            axis_id: {
        ...
        "axis_sources": {axis_id: dict(counter) for axis_id, counter in axis_sources.items()},
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build weak axis pseudo-labels for CAIL2018 GTR v2.")
    ...
    p.add_argument("--limit-per-split", type=int, default=None, help="Optional smoke-test row limit per split.")
...
def main() -> None:
    ...
    if args.limit_per_split is None and not args.force and all(path.exists() for path in expected):
    ...
    stats: Dict[str, Any] = {
        ...
        "schema_file": "cail_gtr_axis_schema.py",
    ...
    for split_name, path in (
        ...
        stats["splits"][split_name] = build_split(
            split_name,
            load_jsonl(path, limit=args.limit_per_split),
            args.output_dir,
    ...
    print(f"Saved CAIL2018 axis labels to {args.output_dir.resolve()}")
