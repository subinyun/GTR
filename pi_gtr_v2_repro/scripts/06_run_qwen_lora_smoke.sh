...
DATA_DIR="output/cail2018_gtr_v2_only/full/qwen3_8b_lora_sft/data_smoke"
if [[ ! -s "${DATA_DIR}/raw_pool_gtr_rerank_train.jsonl" || ! -s "${DATA_DIR}/raw_pool_gtr_rerank_score_prompt_train.jsonl" ]]; then
  ...
  python build_cail_qwen_lora_sft_data.py \
    ...
    --checkpoint output/cail2018_gtr_v2_only/full/hybrid_gtr_v2_best.pt \
    --embedding-cache output/cail2018_gtr_v2_only/full/cache/bge_m3_cail_embeddings_random_trfull_vafull_tefull_seed42.npz \
    --output-dir "${DATA_DIR}" \
