#!/usr/bin/env python3
"""Run Gemini JSON prompts exported by routing experiments."""
...
def make_client(api_key: str) -> tuple[str, Any]:
    try:
    ...
    try:
        ...
        return "google-generativeai", genai_legacy
    except ImportError as exc:
        raise RuntimeError("Install google-genai or google-generativeai to call Gemini.") from exc
    ...
    for attempt in range(max_retries):
        try:
            if client_kind == "google-genai":
                ...
                response = client.GenerativeModel(model).generate_content(prompt, generation_config=generation_config)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prompts", type=Path, default=Path("artifacts/hybrid_gtr_v2_improved/gemini31_gtr_evidence_comparison/prompts_60.jsonl"))
    parser.add_argument("--results", type=Path, default=Path("artifacts/hybrid_gtr_v2_improved/gemini31_gtr_evidence_comparison/results_gemini31_60.jsonl"))
...
def main() -> None:
    ...
    try:
        client_kind, client = make_client(api_key)
    ...
    with args.results.open("a", encoding="utf-8") as handle:
        for ordinal, row in enumerate(prompts, 1):
            ...
            statutes, parse_ok, raw_response = call_gemini(
